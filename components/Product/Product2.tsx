...
const Product2: React.FC<ProductProps> = ({ data, type, style }) => {
    ...
    return (
        <>
            {type === "grid" ? (
                <div className={`product-item grid-type ${style} bg-gradient-to-br from-surface via-white to-surface-pink rounded-3xl playful-shadow fun-hover`}>
                    <div onClick={() => handleDetailProduct(data.id)} className="product-main cursor-pointer block">
                        <div className="product-thumb bg-gradient-to-br from-white to-surface relative overflow-hidden rounded-3xl rainbow-border">
                            {data.new && (
                                <div className="product-tag text-button-uppercase bg-gradient-to-r from-mint-fresh to-ocean-blue text-white px-4 py-1 inline-block rounded-full absolute top-3 left-3 z-[1] animate-bounce font-bold shadow-lg">
                                    ✨ New!
                                </div>
                            )}
                            {data.sale && (
                                <div className="product-tag text-button-uppercase text-white bg-gradient-to-r from-sunset-orange to-candy-red px-4 py-1 inline-block rounded-full absolute top-3 left-3 z-[1] animate-pulse font-bold shadow-lg">
                                    🔥 Sale!
                                </div>
                            )}
                            {style === 'style-1' || style === 'style-3' || style === 'style-4' ? (
                                <div className="list-action-right absolute top-3 right-3 max-lg:hidden">
                                    {style === 'style-4' && (
                                        <div
                                            className={`add-cart-btn w-[40px] h-[40px] flex items-center justify-center rounded-full bg-gradient-to-r from-bubble-pink to-sunset-orange text-white duration-300 relative mb-2 hover:animate-wiggle playful-shadow ${compareState.compareArray.some(item => item.id === data.id) ? 'active' : ''}`}
                                            onClick={e => {
                                                e.stopPropagation();
                                                handleAddToCart()
                                            }}
                                        >
                                            <div className="tag-action bg-gradient-to-r from-bubble-pink to-ocean-blue text-white caption2 px-2 py-1 rounded-full font-bold">🛒 Add To Cart</div>
                                            <Icon.ShoppingBagOpen size={20} className="hover:animate-bounce" />
                                        </div>
                                    )}
                                    <div
                                        className={`add-wishlist-btn w-[40px] h-[40px] flex items-center justify-center rounded-full bg-gradient-to-r from-lavender-purple to-bubble-pink text-white duration-300 relative hover:animate-pulse playful-shadow ${wishlistState.wishlistArray.some(item => item.id === data.id) ? 'active' : ''}`}
                                        onClick={(e) => {
                                            e.stopPropagation()
                                            handleAddToWishlist()
                                        }}
                                    >
                                        <div className="tag-action bg-gradient-to-r from-lavender-purple to-bubble-pink text-white caption2 px-2 py-1 rounded-full font-bold">💖 Add To Wishlist</div>
                                        {wishlistState.wishlistArray.some(item => item.id === data.id) ? (
                                            <>
                                                <Icon.Heart size={18} weight='fill' className='text-sunshine-yellow animate-pulse' />
                                            </>
                                        ) : (
                                            <>
                                                <Icon.Heart size={18} className='hover:animate-bounce' />
                                            </>
                                        )}
                                    </div>
                                    {/*<div*/}
                                    ...
                                    {style === 'style-3' || style === 'style-4' ? (
                                        <div
                                            className={`quick-view-btn w-[40px] h-[40px] flex items-center justify-center rounded-full bg-gradient-to-r from-sky-blue to-ocean-blue text-white duration-300 relative mt-2 hover:animate-wiggle playful-shadow ${compareState.compareArray.some(item => item.id === data.id) ? 'active' : ''}`}
                                            onClick={(e) => {
                                                e.stopPropagation()
                                                handleQuickviewOpen()
                                            }}
                                        >
                                            <div className="tag-action bg-gradient-to-r from-sky-blue to-ocean-blue text-white caption2 px-2 py-1 rounded-full font-bold">👀 Quick View</div>
                                            <Icon.Eye size={20} className="hover:animate-bounce" />
                                        </div>
                                    ) : <></>}
                                </div>
                            ) : <></>}
                            ...
                            {style === 'style-1' || style === 'style-3' ?
                                <div className={`list-action ${style === 'style-1' ? 'grid grid-cols-2 gap-3' : ''} px-5 absolute w-full bottom-5 max-lg:hidden`}>
                                    {style === 'style-1' && (
                                        <div
                                            className="quick-view-btn w-full text-button-uppercase py-3 text-center rounded-full duration-300 bg-gradient-to-r from-sky-blue to-ocean-blue text-white hover:from-ocean-blue hover:to-sky-blue hover:animate-wiggle playful-shadow font-bold"
                                            onClick={(e) => {
                                                e.stopPropagation()
                                                handleQuickviewOpen()
                                            }}
                                    ...
                                    {data.action === 'add to cart' ? (
                                        <div
                                            className="add-cart-btn w-full text-button-uppercase py-3 text-center rounded-full duration-500 bg-gradient-to-r from-bubble-pink to-sunset-orange text-white hover:from-sunset-orange hover:to-bubble-pink hover:animate-bounce playful-shadow font-bold"
                                            onClick={e => {
                                                e.stopPropagation();
                                                handleAddToCart()
                                            }}
                                        ...
                                        <>
                                            <div
                                                className="quick-shop-btn text-button-uppercase py-3 text-center rounded-full duration-500 bg-gradient-to-r from-mint-fresh to-ocean-blue text-white hover:from-ocean-blue hover:to-mint-fresh hover:animate-wiggle playful-shadow font-bold"
                                                onClick={e => {
                                                    e.stopPropagation();
                                            ...
                                            <div
                                                className={`quick-shop-block absolute left-5 right-5 bg-gradient-to-br from-surface via-white to-surface-pink p-6 rounded-3xl playful-shadow rainbow-border ${openQuickShop ? 'open' : ''}`}
                                                onClick={(e) => {
                                                    e.stopPropagation()
                                                }}
                                            >
                                                <div className="list-size flex items-center justify-center flex-wrap gap-2">
                                                    {data.sizes.map((item, index) => (
                                                        <div
                                                            className={`size-item w-12 h-12 rounded-full flex items-center justify-center text-button bg-gradient-to-r from-sunshine-yellow to-peach border-2 border-bubble-pink hover:animate-bounce playful-shadow font-bold ${activeSize === item ? 'active bg-gradient-to-r from-bubble-pink to-sunset-orange text-white' : ''}`}
                                                            key={index}
                                                ...
                                                <div
                                                    className="button-main w-full text-center rounded-full py-4 mt-4 bg-gradient-to-r from-bubble-pink to-sunset-orange text-white hover:from-sunset-orange hover:to-bubble-pink hover:animate-wiggle playful-shadow font-bold text-lg"
                                                    onClick={() => {
                                                        handleAddToCart()
                        ...
                        <div className="product-infor mt-4 lg:mb-7 bg-gradient-to-br from-surface via-white to-surface-pink p-4 rounded-2xl playful-shadow">
                            <div className="product-sold sm:pb-4 pb-2">
                                <div className="progress bg-gradient-to-r from-surface-pink to-surface-blue h-2 w-full rounded-full overflow-hidden relative rainbow-border">
                                    <div
                                        className={`progress-sold bg-gradient-to-r from-bubble-pink to-sunset-orange absolute left-0 top-0 h-full rounded-full animate-pulse`}
                            ...
                            <div className="product-name text-title duration-300 font-bold text-bubble-pink hover:animate-bounce cursor-pointer">{data.name} ✨</div>
