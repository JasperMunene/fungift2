...
import { getProduct, createCheckout } from '@/lib/shopify-service';
import type { ShopifyProduct, ShopifyVariant } from '@/type/shopify';
...
const mapShopifyToProductType = (shopifyProduct: ShopifyProduct): ProductType => {
    ...
    return {
        ...
        thumbImage: shopifyProduct.images.edges.map(edge => edge.node.url),
        price: price,
        originPrice: price * 1.2, // Add 20% as original price for display purposes
        type: shopifyProduct.productType || 'Product',
        rate: 4.5, // Default rating
        description: shopifyProduct.description,
        variation: shopifyProduct.variants.edges.map(edge => ({
            color: edge.node.title,
            colorImage: edge.node.image?.url || shopifyProduct.images.edges[0]?.node.url,
            image: edge.node.image?.url || shopifyProduct.images.edges[0]?.node.url
        })),
        sizes: ['S', 'M', 'L', 'XL'], // Default sizes
        quantityPurchase: 1,
        category: shopifyProduct.productType || 'Category',
        gender: 'Unisex',
        tags: [],
        features: [],
...
const Default: React.FC<Props> = ({ data, productId }) => {
    ...
    useEffect(() => {
        ...
        let initialProduct = data?.find(product => product.id === productId) as ProductType | null;
        if (!initialProduct) {
            initialProduct = null;
        }
        setProductMain(initialProduct);
        ...
        const fetchProduct = async (): Promise<void> => {
            try {
                ...
                // Map Shopify product to local product type
                const mappedProduct = mapShopifyToProductType(productData);
                setProductMain(mappedProduct);
            } catch (err) {
    ...
    const handleVariantChange = (variantId: string): void => {
        ...
        const variant = shopifyProduct.variants.edges.find(edge => edge.node.id === variantId);
        setSelectedVariant(variant?.node || null);
    };
    ...
    const handleActiveSize = (item: string) => {
        setActiveSize(item);
        // Size selection logic would need to be adjusted based on your variant structure
    }

    const handleIncreaseQuantity = () => {
        if (productMain) {
            const updatedProduct = {...productMain, quantityPurchase: productMain.quantityPurchase + 1};
            setProductMain(updatedProduct);
            updateCart(productMain.id, updatedProduct.quantityPurchase, activeSize, activeColor);
        }
    };
    ...
    const handleAddToCart = async (): Promise<void> => {
        if (!selectedVariant || !productMain) return;
        ...
        try {
            const checkoutUrl = await createCheckout({
                lineItems: [{
                    ...
                    quantity: productMain.quantityPurchase || 1,
                }],
            });
                            ...
                            <div className="list-action mt-6">
                                ...
                                <div className="choose-quantity flex items-center lg:justify-between gap-5 gap-y-3 mt-3">
                                    ...
                                    <div
                                        ...
                                        className={`button-main w-full text-center bg-white text-black border border-black ${addingToCart ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                                    >
                                ...
                                <div className="more-infor mt-6">
                                    ...
                                    <div className="flex items-center gap-1 mt-3">
                                        ...
                                        <div className="text-secondary">{productMain.SKU || '53453412'}</div>
                                    </div>
                                    <div className="flex items-center gap-1 mt-3">
